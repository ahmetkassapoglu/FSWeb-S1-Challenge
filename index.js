...
function fenomenEkle(fenomenler,num,pro,fol,pos,pla) {
  const work = {
    "number": num,
    "profile": pro,
    "followers": fol,
...
function enFenomenler(fenomenler) {
  ...
  for ( let i=0; i< fenomenler.length; i++){
    if (fenomenler[i].followers >= 100000000)
      sonuc.push(fenomenler[i].profile)
  }
  return sonuc ;
}
...
function fenomenGonderimSayisi(feno, pro){
  for (let i =0; i< feno.length ; i++){
    if( feno[i].profile === pro ){
      return feno[i].posts
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,TikTok){
 let filtre = fenomenler.filter((item) =>item.platform === TikTok)
 let enbuyuk = filtre[0]
 for (let i =0; i< filtre.length; i++){
  if ( filtre[i].followers>enbuyuk.posts && filtre[i].isInteger === true){
    enbuyuk = filtre[i]
  }
 }
  let sonuc = enbuyuk.posts
  return sonuc
}
